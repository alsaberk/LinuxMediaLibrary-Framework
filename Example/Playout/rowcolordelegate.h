...
public:
    ...
    void paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const override {
        ...
        if (m_globalProps && m_globalProps->selectfile != -1) {
        ...
        if (hasStatus) {
            if (option.state & QStyle::State_Selected) {
                ...
                // Şimdilik sadece seçim rengini gösterelim veya üzerine bir katman atalım
        ...
        if (hasStatus && !(option.state & QStyle::State_Selected)) {
            if (row < m_globalProps->selectfile) opt.palette.setColor(QPalette::Text, Qt::white);
            else if (row == m_globalProps->selectfile) opt.palette.setColor(QPalette::Text, Qt::black);
        }

        QStyledItemDelegate::paint(painter, opt, index);

        // Sürükle-bırak sırasında hedef satırın altına vurgu çizgisini ekle
        if (m_globalProps && m_globalProps->dropIndicatorRow == row) {
            painter->save();
            QPen pen(QColor(85, 170, 255)); // ProgressBar rengiyle uyumlu (Mavi tonları)
            pen.setWidth(4);
            painter->setPen(pen);
